...
def blog_list(request, tag_slug=None):
   ...
   return render(request,'blog_list.html', {'page': page, 'galleries': galleries, 'categories':categories, 'tag':tag})
...
def blog_detail(request, year, month, day, gallery):
   ...
   similar_posts = Gallery.published.prefetch_related('galleryimages_set').filter(tags__in=gallery_tags_ids).exclude(id=gallery.id)
...
class ArticleYearArchiveView(YearArchiveView):
   ...
   def get_context_data(self, **kwargs):
      ...
      context['gallery'] = Gallery.objects.all().prefetch_related('category','galleryimages_set').filter(created__year=kwargs['year'].year)
      return context
...
class ArticleMonthArchiveView(MonthArchiveView):
   ...
   def get_context_data(self, **kwargs):
      context = super().get_context_data(**kwargs)
      context['gallery'] = Gallery.objects.all().prefetch_related('category','galleryimages_set').filter(created__year=kwargs['month'].year).filter(created__month=kwargs['month'].month)
      return context
...
class ArticleDayArchiveView(DayArchiveView):
   ...
   def get_context_data(self, **kwargs):
      context = super().get_context_data(**kwargs)
      context['gallery'] = Gallery.objects.all().prefetch_related('category','galleryimages_set').filter(created__year=kwargs['day'].year).filter(created__month=kwargs['day'].month).filter(created__day=kwargs['day'].day)
      return context
